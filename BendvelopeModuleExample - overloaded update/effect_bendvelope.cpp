...
//Knob range scaling
#define MAX_ATTACK_MS 1000000
#define MAX_DECAY_MS 1000000
#define MAX_RELEASE_MS 5000000
...
void AudioEffectBendvelope::update(void)
{
	...
	while (p < end)
	{
		...
		case SM_ATTACK:
			...
			if( amp == 255 )
			{
				//Serial.println("entering attack hold");
	...
	//Serial.print("\n");
		...
		// This hits every 8 samples
		// Let the timers know that 1/44100 * 8 seconds have passed
		mainTimeKeeper.uIncrement(18.1);
		shadowTimeKeeper.uIncrement(18.1);
		
		//Serial.println((uint32_t)mainTimeKeeper.uGet(), DEC);
...
void AudioEffectBendvelope::changeAmp( RateParameter& param, uint16_t timeVar, uint8_t stateVar, uint8_t& ampVar )
{
...
uint16_t AudioEffectBendvelope::getDecay( void )
{
  return envDecay.timeScale * 1000;
}
...
//
//  From uCModules on Dec 15th, 2015
//**********************************************************************//
